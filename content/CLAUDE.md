...
后台正式抓取主要使用 `background/background.js` 中注入的 `extractProductRufusData()`，Content 模块负责页面增强和备用提取。链接巡查不在 Content Script 中执行，而由 Background 调用受授权保护的 `amazon-crawler` API。底层 Rufus 命名保留用于兼容 Amazon 旧 DOM 结构。
