...
alexai 是一个 Chrome Manifest V3 扩展，用于批量抓取 Amazon 商品页中的 Alexa for Shopping 提示按钮信息，并兼容 `Ask Rufus` 旧页面结构。项目沿用原来的队列、重试、断点续传和导出机制，但目标页面已经从卖家页切换为商品详情页。High Price 检查读取 Amazon 页面当前展示的价格提示，通常需要本机浏览器已登录 Amazon 账号。
...
- 提取 Amazon `High price` 等价格提示；该检查依赖页面实际展示内容，可能需要本机 Amazon 登录态
...
- High Price 是 Amazon 动态/个性化展示，不是稳定公开字段；未登录本机 Amazon 账号、账号地区不匹配或页面未命中实验时可能无法检测。
...
- 本项目仅用于学习研究和个人数据整理，不保证抓取结果完整、准确或持续可用。
